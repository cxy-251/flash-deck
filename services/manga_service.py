...
# （第一个已配置资源库根路径 + media_library），改根路径配置就全局跟着变。
MEDIA_LIBRARY_DIR = get_primary_dir("media_library")
MANGA_DIR = os.path.join(MEDIA_LIBRARY_DIR, "manga")
...
def _media_roots(dir_flag: str) -> List[str]:
    """某一类库（"manga"/"novels"）在所有地方的候选目录：主库（新下载落地的地方，
    排最前面）+ 已配置资源库根路径（app_config.py：SD 卡等）下的 media_library/<name> +
    omni-deck 自己目录下的旧版位置（迁移期间兼容，等东西搬空了这条自然就没内容了）。"""
    primary = MANGA_DIR if dir_flag == 'manga' else NOVELS_DIR
    dirs = [primary]
    for d in find_library_dirs("media_library", dir_flag):
        if d not in dirs:
            dirs.append(d)
    legacy = os.path.join(SCRIPT_DIR, dir_flag)
...
def _sanitize_for_json(obj):
    """递归清理内部状态字典里不能/不该序列化成 JSON 的部分（下划线私有字段、Queue 对象）。

    Args:
        obj: 任意嵌套的 dict/list/标量值。

    Returns:
        清理后的同构结构，可以安全传给 json.dumps。
    """
...
def find_existing_cbz_by_aid(aid: str, target_dir_path: str = MANGA_DIR) -> Optional[str]:
    ...
    if not aid_str:
        return None
    dir_flag = _classify_dir_flag(os.path.join(target_dir_path, "x"))
