"""
sc2_panel_service.py - 星际争霸 2 对战面板后端服务

首页顶级卡片"⭐ 星际争霸2"背后的数据 + 对局控制层。地图缩略图抽取/去重、
快捷键安装、拉起对局全部是本项目自己的代码（sc2_runner.py / 本文件），
装在 omni-deck 自己的 venv 里（burnysc2 / s2clientprotocol / protobuf==3.20.3 等）。

唯一还跨项目 import 的是 ~/Games/claude/omniMod/sc2Mod/bake.py —— 那个项目现在专注"做 mod"：
...
HERE = Path(__file__).resolve().parent.parent  # 本文件在 services/ 下，项目根路径是上一级
SC2MOD_DIR = os.environ.get("SC2MOD_DIR", os.path.expanduser("~/Games/claude/omniMod/sc2Mod"))
SC2_DIR = Path(os.environ.get("SC2_DIR", os.path.expanduser("~/Games/StarCraft II")))
SC2_MAPS_DIR = SC2_DIR / "Maps"

THUMB_CACHE = Path.home() / ".cache/sc2mod/thumbs_v3"
...
def sync_mods_to_pfx() -> None:
    ...
    try:
        src_dir = Path(os.environ.get("SC2_DIR", os.path.expanduser("~/Games/StarCraft II"))) / "Mods"
...
def open_editor() -> tuple[bool, str]:
    ...
    proton_wine = HERE / "bin" / "proton-wine"
...
def start_match(map_stem: str, race: str, opponents: List[Dict[str, str]], mods: List[str]) -> tuple[bool, str]:
    """在后台线程里起一局（子进程跑本项目 venv 里的 sc2_runner.py），立即返回不阻塞。
    ...
    def _worker():
        """在独立线程里同步 hotkey/mod、拉起 sc2_runner.py 子进程并等待结果，跑完更新 _state。"""
        install_hotkeys()
        sync_mods_to_pfx()
        cmd = [_UV, "run", "python", "sc2_runner.py",
        ...
        tail = ""
        try:
            proc = subprocess.run(cmd, cwd=str(HERE), capture_output=True, text=True, timeout=3 * 3600)
