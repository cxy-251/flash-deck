...
CONFIG_PATH = paths.PRIVACY_FILE
...
def _load_or_init_config() -> Dict[str, Any]:
    ...
    # 首次初始化：用 settings.json 里的 nsfw_default_password 作为初始密码
    salt = secrets.token_hex(8)
    config = {
        "salt": salt,
        "password_hash": _hash_password(settings.get("nsfw_default_password"), salt),
