# -*- coding: utf-8 -*-
"""
Omni Deck MEGA 网盘原生集成服务 (基于 mega-cmd 原生引擎)

职责架构：
1. 原生服务守护：管理与连接 <tools.mega_cmd_dir>/bin/mega-cmd-server
2. 账户凭证管理：支持账号密码登录、2FA验证、状态查询与安全登出
3. 网盘资源浏览：基于 mega-ls 实现云端文件树解析、面包屑路径与属性提取
4. 极速异步下载：调度 mega-get 后台下载到收件箱（settings.inbox_dir，默认 ~/Downloads）
...
def _mega_dir(*parts: str) -> str:
    """mega-cmd 安装目录（settings.tools.mega_cmd_dir）下的路径。"""
    return os.path.join(settings.tool("mega_cmd_dir"), *parts)


def _download_dir() -> str:
    return settings.get("inbox_dir")
...
def get_mega_env() -> Dict[str, str]:
    ...
    env["PATH"] = f"{_mega_dir('bin')}:{current_path}"
    
    current_ld = env.get("LD_LIBRARY_PATH", "")
    env["LD_LIBRARY_PATH"] = f"{_mega_dir('lib')}:{current_ld}"
    return env

def ensure_server_running() -> bool:
    """确保 mega-cmd-server 后台服务正常存活并在运行"""
    socket_path = os.path.join(paths.HOME, ".megaCmd", "megacmd.socket")
    try:
    ...
    try:
        env = get_mega_env()
        subprocess.Popen([_mega_dir("bin", "mega-cmd-server"), "--do-not-log-to-stdout"], env=env, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True)
        for _ in range(30):
...
def run_mega_command(args: List[str], timeout: int = 35) -> Dict[str, Any]:
    ...
    cmd = [_mega_dir("bin", "mega-exec")] + args
    try:
        proc = subprocess.run(
            cmd,
            env=env,
...
def start_download(source_path_or_link: str, target_location: str = "downloads") -> Dict[str, Any]:
    ...
    run_mega_command(["lcd", download_dir], timeout=5)

    args = ["get", "-q", "-m", "--ignore-quota-warn", src]
    # 关键防重名处理：
    # 仅当确定为网盘内部单文件时，显式拼接具体目标文件名 (<收件箱>/filename)；
    # 若为公开分享直链 (http...) 或目录，绝对不追加收件箱路径参数，
    ...
    if not src.startswith("http") and not src.endswith('/'):
        ...
        if fname:
            args.append(os.path.join(download_dir, fname))
    ...
    if res["success"]:
        msg = f"任务已加入后台下载队列，将保存至 {download_dir}"
        if quota_wait:
