...
# 条目的 rel_path 用「逻辑路径」：<前缀>/<在该类目录下的相对路径>，跟书在哪个资源库、哪块盘无关——
# 书从 SSD 挪到 SD 卡，阅读进度（localStorage 以 rel_path 为键）照样对得上。
#   docs/Film/xxx.md            -> 各资源库的 media_library/docs/Film/xxx.md
#   novels/standard/分类/x.epub -> 各资源库的 media_library/novels/standard/分类/x.epub
LOGICAL_ROOTS = {"docs": "media.docs", "novels": "media.novels"}
...
def novels_standard_dir() -> str:
    return library.primary("media.novels.standard")


def novels_nsfw_dir() -> str:
    return library.primary("media.novels.nsfw")
...
def _roots(key: str) -> List[str]:
    """某一类目录在所有在线库里的位置，默认库排最前。"""
    primary = library.primary(key)
    out = [primary] if os.path.isdir(primary) else []
...
def docs_roots() -> List[str]:
    return _roots("media.docs")


def to_logical(full_p: str) -> str:
    """磁盘绝对路径 -> 逻辑路径（不在文档/小说目录里时退回文件名）。"""
    real = os.path.realpath(full_p)
    for prefix, key in LOGICAL_ROOTS.items():
        for base in _roots(key):
            rb = os.path.realpath(base)
            if real.startswith(rb + os.sep):
                return prefix + "/" + os.path.relpath(real, rb).replace("\\", "/")
    return os.path.basename(full_p)


def _inside_allowed(full_p: str) -> bool:
    real = os.path.realpath(full_p)
    for key in LOGICAL_ROOTS.values():
        for base in _roots(key):
            if real.startswith(os.path.realpath(base) + os.sep):
                return True
    return False


def is_doc_path(full_p: str) -> bool:
    real = os.path.realpath(full_p or "")
    return any(real.startswith(os.path.realpath(b) + os.sep) for b in docs_roots())


def get_queue_file(is_nsfw: bool = False) -> str:
    """获取对应文库类型的待下载队列文件路径"""
    return os.path.join(_ensure(novels_nsfw_dir() if is_nsfw else novels_standard_dir()), ".download_queue.json")


def get_temp_dir(is_nsfw: bool = False) -> str:
    """获取对应文库类型的临时文件目录"""
    return _ensure(os.path.join(novels_nsfw_dir() if is_nsfw else novels_standard_dir(), ".temp"))


def nsfw_cache_dir() -> str:
    return _ensure(os.path.join(novels_nsfw_dir(), ".cache"))


def classics_cache_dir() -> str:
    return _ensure(os.path.join(novels_standard_dir(), ".classics_cache"))
...
def get_novels_library(q: str = "", is_nsfw: bool = False) -> List[Dict[str, Any]]:
    ...
    if is_nsfw:
        target_dirs = [(d, False) for d in _roots("media.novels.nsfw")]
    else:
        target_dirs = [(d, False) for d in _roots("media.novels.standard")]
        target_dirs += [(d, True) for d in _roots("media.novels")]
    seen_dirs = set()
    dedup_dirs = []
    ...
    def _on_progress(done, total):
        ...
        events.broadcast({'type': 'library_indexed', 'dir': 'novels', 'done': done, 'total': total})
...
def get_docs_explorer(sub_dir: str = "", q: str = "", doc_filter: str = "all") -> Dict[str, Any]:
    """
    资源管理器式技术文档浏览（解决数千文档全量加载卡顿）：
    - 按文件夹层级展示，只读当前目录的直接子项；多个资源库里的同名目录合并显示
    - 带关键词时在当前目录下递归检索
    """
    ...
    if safe_rel in ('.', '/') or safe_rel.startswith('..'):
        safe_rel = ''
    current_dirs = [os.path.join(r, safe_rel) if safe_rel else r for r in roots]
    ...
    accum = ''
    for part in [x for x in safe_rel.split('/') if x]:
    ...
    def doc_ext(fname):
        ...
        if ext not in ('.md', '.rst', '.markdown', '.txt'):
            return None
        clean = 'md' if ext in ('.md', '.markdown') else ext.lstrip('.')
        return clean if (doc_filter == 'all' or clean == doc_filter) else None
        ...
        for base in current_dirs:
            for root, _, fnames in os.walk(base):
                for fname in sorted(fnames):
                    clean_ext = doc_ext(fname) if not fname.startswith('.') else None
                    base_title = os.path.splitext(fname)[0]
                    if not clean_ext or (q_clean not in base_title.lower() and q_clean not in fname.lower()):
                        continue
                    full_p = os.path.join(root, fname)
                    rel_p = to_logical(full_p)
                    if rel_p in seen:
                    ...
                    try:
                        ...
                        continue
                    dir_rel = os.path.dirname(rel_p)[len('docs/'):] if rel_p.startswith('docs/') else ''
                    matched.append({
                        'type': 'file', 'name': fname, 'title': base_title, 'ext': clean_ext,
                        'rel_path': rel_p, 'dir_rel': dir_rel,
                        'size_kb': round(st.st_size / 1024, 1),
                        'mtime': time.strftime('%Y-%m-%d %H:%M', time.localtime(st.st_mtime)),
                    })
        return {'is_search': True, 'q': q, 'current_dir': safe_rel, 'breadcrumbs': breadcrumbs,
                'parent_dir': parent_dir, 'folders': [], 'files': matched, 'total_items': len(matched)}
    ...
    for base in current_dirs:
        ...
        for entry in entries:
            ...
            try:
                st = os.stat(full_entry)
                mtime_str = time.strftime('%Y-%m-%d %H:%M', time.localtime(st.st_mtime))
                if os.path.isdir(full_entry):
                    seen.add(entry)
                    folders.append({
                        'type': 'dir', 'name': entry,
                        'path': f"{safe_rel}/{entry}" if safe_rel else entry,
                        'items_count': len([x for x in os.listdir(full_entry) if not x.startswith('.')]),
                        'mtime': mtime_str,
                    })
                elif os.path.isfile(full_entry):
                    clean_ext = doc_ext(entry)
                    if not clean_ext:
                        continue
                    seen.add(entry)
                    files.append({
                        'type': 'file', 'name': entry, 'title': os.path.splitext(entry)[0], 'ext': clean_ext,
                        'rel_path': to_logical(full_entry),
                        'size_kb': round(st.st_size / 1024, 1), 'mtime': mtime_str,
                    })
            except OSError:
                pass
    folders.sort(key=lambda f: f['name'])
    files.sort(key=lambda f: f['name'])
    return {'is_search': False, 'current_dir': safe_rel, 'breadcrumbs': breadcrumbs, 'parent_dir': parent_dir,
            'folders': folders, 'files': files, 'total_items': len(folders) + len(files)}
...
GUTENBERG_ZH_CATALOG_FILE = os.path.join(paths.CATALOGS, 'gutenberg_zh_catalog.json')
...
XBOOKCN_CATALOG_FILE = os.path.join(paths.CATALOGS, 'xbookcn_catalog.json')
...
def start_download_novel_task(novel_id: str, title: str, author: str = "佚名", intro: str = "", cover_url: str = "", is_nsfw: bool = False) -> Dict[str, Any]:
    ...
    target_dir = _ensure(novels_nsfw_dir() if is_nsfw else novels_standard_dir())
...
def resolve_novel_or_doc_path(rel_path: str) -> Optional[str]:
    """逻辑路径（docs/... 或 novels/...）-> 磁盘绝对路径；结果一定落在各库的文档/小说目录内，
    带 .. 或指向别处的一律返回 None。找不到原路径时按文件名在这些目录里兜底查找一次。"""
    if not rel_path:
        return None
    norm = os.path.normpath(rel_path.replace('\\', '/')).lstrip(os.sep)
    parts = norm.split(os.sep)
    if '..' in parts:
        return None
    prefix, rest = parts[0], os.path.join(*parts[1:]) if len(parts) > 1 else ''
    if prefix in LOGICAL_ROOTS and rest:
        for base in _roots(LOGICAL_ROOTS[prefix]):
            cand = os.path.join(base, rest)
            if os.path.isfile(cand) and _inside_allowed(cand):
                return cand
    fname = os.path.basename(norm)
    search_keys = ["media.docs", "media.novels"]
