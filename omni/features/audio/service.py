"""
Omni Deck 音声与广播剧流媒体服务
特性：
1. 扫描所有资源库的 media_library/audio/{standard,nsfw} (常规有声书与 NSFW 专区物理隔离)
...
from omni.core import library, media_index
...
def _audio_root_dirs() -> List[str]:
    """各库 media_library/audio 根目录——根下直接放的零散文件归到常规有声书里。"""
    return library.dirs("media.audio")


def get_audio_dirs(is_nsfw: bool = False) -> List[str]:
    """指定模式下所有在线资源库里存在的音频目录（优先级顺序）。"""
    if is_nsfw:
        return library.dirs("media.audio.nsfw")
    return library.dirs("media.audio.standard") + _audio_root_dirs()
...
def _iter_audio_files_in_dir(adir: str):
    ...
    for root, dirs, fnames in os.walk(adir):
        ...
        if root == adir and adir in _audio_root_dirs():
