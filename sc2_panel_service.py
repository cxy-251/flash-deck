...
唯一还跨项目 import 的是 ~/Games/claude/sc2Mod/bake.py —— 那个项目现在专注"做 mod"：
...
def start_match(map_stem: str, race: str, opponents: List[Dict[str, str]], mods: List[str]) -> tuple[bool, str]:
    """后台线程起一局（子进程跑本项目自己 venv 里的 sc2_runner.py），非阻塞。
    opponents: [{"race": "T", "difficulty": "cheatinsane"}, ...]，1 个起，全是电脑——玩家只有你自己。"""
    if not opponents:
